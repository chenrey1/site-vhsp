...
class Dashboard extends G_Controller
{
    ...
    public function transferBalance()
    {
        ...
        $sender_id = $this->session->userdata('info')['id'];
        $sender = $this->db->where('id', $sender_id)->get('user')->row();

        // Alıcı kullanıcıyı e-mail ile bul
        $recipient = $this->db->where('email', $recipient_email)->get('user')->row();

        // Hata kontrolleri
        if (!$recipient) {
            flash('Hata', 'Alıcı kullanıcı bulunamadı. Lütfen geçerli bir e-mail adresi girin.');
            redirect(base_url('client/balance'), 'refresh');
            return;
        }
        ...
        if ($amount <= 0) {
            flash('Hata', 'Transfer miktarı 0\'dan büyük olmalıdır.');
            redirect(base_url('client/balance'), 'refresh');
            return;
        }

        if ($sender->balance < $amount) {
            flash('Hata', 'Yeterli bakiyeniz bulunmamaktadır.');
            redirect(base_url('client/balance'), 'refresh');
            return;
        }
        ...
        $this->db->where('id', $sender_id)->update('user', ['balance' => $sender_balance_after]);

        // Alıcı bakiyesini artır
        $recipient_balance_before = $recipient->balance;
        $recipient_balance_after = $recipient_balance_before + $amount;
        $this->db->where('id', $recipient->id)->update('user', ['balance' => $recipient_balance_after]);

        // Transfer işlemi için benzersiz ID oluştur
        $transfer_id = 'TR' . time() . rand(1000, 9999);

        // Gönderen için işlem kaydı oluştur
        $sender_transaction = [
            'user_id' => $sender_id,
            ...
            'description' => $description ? $description : $recipient_email . ' kullanıcısına transfer',
        ...
        // Alıcı için işlem kaydı oluştur
        $recipient_transaction = [
            ...
            'description' => $description ? $description : $sender->email . ' kullanıcısından transfer',
            'related_id' => $transfer_id,
        ...
        flash('Başarılı', 'Transfer işlemi başarıyla gerçekleştirildi.');
        redirect(base_url('client/balance'), 'refresh');
    }
    ...
    public function buyOnBalance()
    {
        ...
        // Oturum kontrolü
        if (!isset($this->session->userdata('info')['id'])) {
            flash('Ups.', 'Oturum süreniz dolmuş. Lütfen tekrar giriş yapın.');
            redirect(base_url(), 'refresh');
            exit;
        }

        // Sepet ve kullanıcı bilgilerini al
        ...
        // Sepet boş kontrolü
        if (empty($cart)) {
            flash('Ups.', 'Sepetiniz boş. Lütfen satın almak istediğiniz ürünleri sepete ekleyin.');
            redirect(base_url(), 'refresh');
            exit;
        }

        // Ödeme işleminin toplam tutarını hesapla
        $price = $this->M_Payment->calculate($encode);
        $properties = $this->db->where('id', 1)->get('properties')->row();

        addlog('buyOnBalance', 'Bakiye ile satın alma işlemi başlatıldı. Sepet içeriği: ' . $encode . ' Mevcut bakiye: ' . $user->balance . ' Toplam tutar: ' . $price);

        // Bakiye kontrolü
        if ($user->balance < $price) {
            flash('UPS!', 'Bakiyeniz Yetersiz. Güncel bakiyeniz: ' . number_format($user->balance, 2) . ' TL');
            addlog('buyOnBalance', 'Bakiye yetersiz. Satın alma işlemi iptal edildi. Kullanıcı ID: ' . $user->id . ' Bakiye: ' . $user->balance . ' Tutar: ' . $price);
        ...
        try {
            ...
            if (!$order_id) {
                ...
                flash('Hata', 'Sipariş oluşturulamadı. Lütfen tekrar deneyin.');
                addlog('buyOnBalance', 'Sipariş oluşturulamadı. Kullanıcı ID: ' . $user->id);
                redirect(base_url('client'));
                return;
            }
            ...
            if ($this->db->trans_status() === FALSE) {
                $this->db->trans_rollback();
                flash('Hata', 'İşlem sırasında bir hata oluştu. Lütfen tekrar deneyin.');
                addlog('buyOnBalance', 'Transaction başarısız. Kullanıcı ID: ' . $user->id . ' Sipariş ID: ' . $shop->id);
                redirect(base_url('client'));
                return;
            }
            ...
            // Siparişi işle
            $result = $this->M_Payment->confirmShopForCart($shop->id);

            if (!$result) {
                flash('Uyarı', 'Sipariş işleme sırasında bir sorun oluştu. Lütfen destek ekibiyle iletişime geçin.');
                addlog('buyOnBalance', 'Sipariş onaylama başarısız. Kullanıcı ID: ' . $user->id . ' Sipariş ID: ' . $shop->id);
                redirect(base_url('client'));
                return;
            }
            ...
            addlog('buyOnBalance', 'Satın alım tamamlandı. Kullanıcı ID: ' . $user->id . ' Sipariş ID: ' . $shop->id . ' Yeni bakiye: ' . $user->balance);
            flash('Başarılı', 'Satın alımınız tamamlandı. Siparişlerinizi "Ürünlerim" sayfasından takip edebilirsiniz.');
            redirect(base_url('client'));

        } catch (Exception $e) {
            $this->db->trans_rollback();
            flash('Hata', 'İşlem sırasında bir hata oluştu. Lütfen tekrar deneyin.');
            addlog('buyOnBalance', 'Exception: ' . $e->getMessage() . ' Kullanıcı ID: ' . $user->id);
            redirect(base_url('client'));
            return;
        }
    }
    ...
    public function showTicket($id)
    {
        ...
        if ($ticket) {
            ...
            $this->load->view('theme/' . $this->properties->theme . '/includes/header', $data);
            ($this->properties->theme != "torius") ? $this->load->view('theme/' . $this->properties->theme . '/client/includes/sidebar', $data) : NULL;
            $this->load->view('theme/' . $this->properties->theme . '/client/show-ticket', $data);
            $this->load->view('theme/' . $this->properties->theme . '/includes/footer', $data);
    ...
    {
        if ($this->properties->api_is_active == 0) {
            flash('Hata.', 'Aradığınız sayfa bulunamadı.');
        ...
        $this->clientView('api_settings', $data);
    }
    ...
    // Kullanıcının bayilik bilgilerini görüntüleyen fonksiyon
    public function my_dealer()
    {
        $sideMenu = true;
        $user_id = $this->session->userdata('info')['id'];
        $this->load->model('M_Dealer');

        // Kullanıcının bayilik bilgilerini al
        ...
        $data = [
            ...
            'has_pending_application' => $this->hasPendingDealerApplication($user_id)
        ];

        // Eğer bayilik bilgisi varsa, bir sonraki bayilik seviyesini ve işlem geçmişini de gönder
        if ($dealer_info) {
            // Kullanıcının bayilik geçmişi
            $data['dealer_history'] = $this->M_Dealer->getUserDealerHistory($user_id);

            // Bir sonraki bayilik seviyesi
            $this->db->where('status', 1);
            $this->db->where('min_purchase_amount >', $dealer_info->total_purchase);
            $this->db->order_by('min_purchase_amount', 'ASC');
            $this->db->limit(1);
            $data['next_dealer_type'] = $this->db->get('dealer_types')->row();

            // Kullanıcının süreli bayilik değişim planlaması kontrolü
            $this->db->where('user_id', $user_id);
            $this->db->where('is_processed', 0);
            $data['timed_change'] = $this->db->get('dealer_timed_changes')->row();
        }

        $this->clientView('my-dealer', $data, $sideMenu);
    }

    /**
     * Bayilik başvuru formunu işler
     */
    public function submit_dealer_application()
    {
        addlog('submit_dealer_application', 'Bayilik başvurusu yapıldı');

        $user_id = $this->session->userdata('info')['id'];
        $this->load->model('M_Dealer');

        // Kullanıcının zaten aktif bir bayiliği var mı kontrol et
        $existing_dealer = $this->M_Dealer->getUserDealerInfo($user_id);
        if ($existing_dealer) {
            flash('Bilgi', 'Zaten aktif bir bayiliğiniz bulunmaktadır.');
            redirect(base_url('client/dashboard/my_dealer'), 'refresh');
            return;
        }

        // Form doğrulama kurallarını ayarla
        $this->load->library('form_validation');
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->session->set_flashdata('message', validation_errors());
            redirect(base_url('client/my_dealer'), 'refresh');
            return;
        }

        // Başvuru bilgilerini hazırla
        $application_data = [
            'user_id' => $user_id,
            ...
            'description' => $this->input->post('description'),
            'estimated_revenue' => $this->input->post('estimated_revenue') ? $this->input->post('estimated_revenue') : 0,
            'status' => 'pending', // Başvuru durumu: beklemede
            'created_at' => date('Y-m-d H:i:s')
        ];

        // Başvuruyu veritabanına ekle
        $this->db->insert('dealer_applications', $application_data);

        // Otomatik onay ayarını kontrol et ve işle
        $this->M_Dealer->checkPendingApplications();

        flash('Başarılı', 'Bayilik başvurunuz alınmıştır. En kısa sürede incelenip size dönüş yapılacaktır.');
        redirect(base_url('client/my_dealer'), 'refresh');
    }

    // Kullanıcının bekleyen bayilik başvurusu olup olmadığını kontrol eder
    private function hasPendingDealerApplication($user_id)
    {
        $this->db->where('user_id', $user_id);
        $this->db->where('status', 'pending');
        $result = $this->db->get('dealer_applications');
    ...
    public function draws()
    {
        ...
        // Aktif Çekilişler
        $draws = $this->M_Draw->get_all_draws(false);

        // Çekilişlere ödül ve katılımcı sayısı bilgilerini ekle
        foreach ($draws as &$draw) {
            // Ödülleri getir - artık ürün bilgileri de geliyor
            $draw->rewards = $this->M_Draw->get_rewards($draw->id);

            $draw->participant_count = $this->M_Draw->get_participant_count($draw->id);

            // Kullanıcı oturum açmışsa, katılım durumunu kontrol et
            $draw->joined = false;
            if (isset($this->session->userdata('info')['id'])) {
                $user_id = $this->session->userdata('info')['id'];
                $draw->joined = $this->M_Draw->is_user_joined($draw->id, $user_id);
            }
        }

        // Geçmiş Çekilişler (biten çekilişler)
        $past_draws = $this->db->where('status', 2)->get('draws')->result();

        // Geçmiş çekilişlere ödül ve katılımcı sayısı bilgilerini ekle
        ...
        // Son katılınan çekilişi session'dan al
        $last_joined_draw = $this->session->flashdata('last_joined_draw');

        $data = [
            'draws' => $draws,
            'past_draws' => $past_draws,
            'last_joined_draw' => $last_joined_draw,
            'properties' => $this->db->where('id', 1)->get('properties')->row(),
    ...
    // Çekilişe katıl
    public function join_draw($draw_id)
    {
        // CSRF koruması
        $this->load->library('form_validation');
        $this->form_validation->set_rules('csrf_token', 'CSRF', 'trim|required');

        if ($this->form_validation->run() === FALSE && $this->input->post()) {
            flash('Hata', 'Güvenlik doğrulaması başarısız. Lütfen tekrar deneyin.');
            redirect('client/draws');
            return;
        }
        ...
        $draw_id = (int) $draw_id;
        if ($draw_id <= 0) {
            flash('Hata', 'Geçersiz çekiliş ID.');
            redirect('client/draws');
            return;
        }
        ...
        // Log için çekiliş bilgilerini al
        $draw = $this->M_Draw->get_draw($draw_id);
        if (!$draw) {
            flash('Hata', 'Çekiliş bulunamadı.');
            redirect('client/draws');
            return;
        }

        // Çekilişe katılım denemesini logla
        $this->load->helper('log');
        draw_info('join_draw', 'Çekilişe katılım denemesi', [
            'user_id' => $user_id,
            'draw_id' => $draw_id,
            'draw_name' => $draw->name
        ]);
        ...
        // Katılım işlemi
        $join_result = $this->M_Draw->join_draw($draw_id, $user_id);

        if ($join_result['success']) {
            // Başarılı katılım bilgilerini logla
            draw_success('join_draw', 'Çekilişe başarıyla katılındı', [
                'user_id' => $user_id,
                'draw_id' => $draw_id,
                'draw_name' => $draw->name
            ]);

            // Son katılınan çekilişi oturuma kaydet
            $this->session->set_userdata('last_joined_draw', [
                'id' => $draw->id,
                'name' => $draw->name,
                'end_time' => $draw->end_time
            ]);

            flash('Başarılı', $join_result['message']);
        } else {
            // Başarısız katılım bilgilerini logla
            draw_error('join_draw', 'Çekilişe katılım başarısız', [
                'user_id' => $user_id,
                'draw_id' => $draw_id,
                'draw_name' => $draw->name,
                'error' => $join_result['message']
            ]);

            flash('Hata', $join_result['message']);
        }

        redirect('client/draws');
    }

    // Kazanılan çekiliş ödülleri ve katıldığı çekilişler
    public function draw_rewards()
    {
        $this->load->model('M_Draw');
        $user_id = $this->session->userdata('info')['id'];

        // Tüm ödüller (bakiye ve ürün) - model üzerinden ürün bilgileri de gelecek
        $won_rewards = $this->M_Draw->get_user_rewards($user_id);

        // Toplam kazanılan bakiye hesapla
        $total_won_balance = 0;
        foreach ($won_rewards as $reward) {
            if ($reward->type === 'bakiye' && isset($reward->amount)) {
                $total_won_balance += floatval($reward->amount);
            }
        }

        // Katılınan çekilişleri getir
        $joined_draws = $this->M_Draw->get_user_joined_draws($user_id);

        $data = [
            'won_rewards' => $won_rewards,
            'joined_draws' => $joined_draws,
            'total_won_balance' => $total_won_balance,
            'properties' => $this->db->where('id', 1)->get('properties')->row(),
            'category' => getActiveCategories(),
            'pages' => $this->db->get('pages')->result(),
            'footerBlog' => $this->db->limit(3)->order_by('id', 'DESC')->get('blog')->result(),
        ...
        $this->clientView('draw-user-rewards', $data);
    }

    // Çekiliş detayları ve katılımcıları (halka açık)
    public function draw_detail($draw_id)
    {
        $this->load->model('M_Draw');
        ...
        if (isset($this->session->userdata('info')['id'])) {
            $user_id = $this->session->userdata('info')['id'];
            $joined = $this->M_Draw->is_user_joined($draw_id, $user_id);
        }

        // Katılımcı sayısını getir
        $participant_count = $this->M_Draw->get_participant_count($draw_id);

        // Ödülleri getir - artık ürün bilgilerini de içeriyor
        $rewards = $this->M_Draw->get_rewards($draw_id);

        // Kazananları getir (çekiliş bitmişse)
        $winners = [];
        if ($draw->status == 2) {
            $winners = $this->M_Draw->get_draw_results($draw_id);
        }

        // Anonim katılımcı listesi
        $participant_names = $this->M_Draw->get_draw_participant_names($draw_id);
        ...
        $this->clientView('draw-detail', $data);
    }
}
