...
class M_Earnings extends CI_Model
{
    ...
    private function processProductSale($shop, $invoice)
    {
        ...
        if (!$shop->seller_id) {
            $site_earning_data = array(
                ...
                'shop_id' => $shop->id,
                'amount' => $invoice->price - $payment_commission_amount,
                'total' => $invoice->price,
                'payment_method' => $shop->type,
                ...
                'commission_amount' => 0,
                'commission_rate' => 0,
                'net_earnings' => $invoice->price - $payment_commission_amount,
    ...
    public function updateEarningByInvoice($invoice_id, $data)
    {
        ...
        if ($earning) {
            return $this->db->where('id', $earning->id)->update('earnings', $data);
        } else {
            // Eğer earnings kaydı yoksa, invoice bilgilerini alıp yeni bir kayıt oluştur
            ...
            if ($invoice) {
                ...
                if ($shop) {
                    $earning_data = array(
                        ...
                        'payment_commission_amount' => $invoice->payment_commission,
                        'payment_commission_rate' => getCommission($shop->user_id),
                        'commission_amount' => 0,
                        'commission_rate' => 0,
                        'net_earnings' => $invoice->price - $invoice->payment_commission,
                        'description' => $data['description'] ?? "Ürün satışı. Fatura ID: " . $invoice->id,
                        'seller_type' => $invoice->seller_id ? 'user' : 'site',
                        'payment_type' => 'Satın alım'
                    );
